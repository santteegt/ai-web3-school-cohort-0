...
> **Assigned issues:** #1, #2, #8, #9 (Day 10 — base infrastructure vertical)
> **Created:** 2026-06-10
...
Clone/pull the repo, then read these in order. They are the complete working contract —
do not invent anything that contradicts them:

1. hackathon/guild-os/CLAUDE.md      — Component Map (canonical class + file names),
                                        build rules, the Don't list, the When-Unsure
                                        decision shortcuts, and the Sprint Day Gates.
...
3. hackathon/guild-os/docs/TECH_STACK.md   — Locked library versions; read before adding
                                        any import. Update requirements.txt if you add a
                                        package, never silently.
4. hackathon/guild-os/docs/MVP_FLOW.md     — The 15-step coordination loop. Every line of
                                        code must map to one of these steps. If it doesn't,
                                        it's out of scope — do not build it.
5. hackathon/guild-os/docs/RISKS.md        — Decision Log + fallbacks are ALREADY decided.
                                        Do not re-evaluate them. When a primary path fails,
                                        switch to the documented fallback and log it.
6. hackathon/guild-os/docs/VALIDATION_PLAN.md  — Definition of done per integration. Each
                                        issue names the exact sections that gate it.
...
- hackathon/research/               — Deep-dive analyses behind locked decisions (e.g.
                                      ...
                                      RISKS.md says a decision is "already made," the
                                      reasoning usually lives here. Read to understand a
                                      fallback, not to relitigate it.
...
NOT expand your scope: only the 15-step MVP_FLOW and your four issues define what to build.
...
- NETWORK: Base mainnet (chain_id 8453) ONLY. Never Ethereum mainnet, never Base Sepolia.
  Explorer links are https://basescan.org/tx/...
...
- COMPONENT MAP: Use the exact class names, file paths, and the 7 MCP tool names from
  CLAUDE.md. Do not rename, do not add parallel modules.
- HUMAN GATES: Every gate (0, 0.5, 1, 2) must HALT and wait for an explicit `y`. Never
...
2. The Validation Plan sections the issue names are satisfiable — update their status
   boxes ([ ] → [x]) in docs/VALIDATION_PLAN.md as part of the PR.
...
For each issue, work on a dedicated branch and open a PR against `main`:
  ...
  PR body MUST contain, in this order:
    ...
    ## Validation plan
    The VALIDATION_PLAN.md sections this PR advances, with their new status.
...
- The hard-constraints block mirrors `hackathon/guild-os/CLAUDE.md` — Base mainnet only,
  no secrets in source, exact Component Map names, halting human gates, and the ERC-8004
  `giveFeedback()` caller rule (RISKS §F2). These are the most common review-failure modes.
